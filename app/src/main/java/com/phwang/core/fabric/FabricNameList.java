...
public class FabricNameList {
    ...
    protected void updateNameList() {
        ...
        this.nameListTagStr_ = Encoders.iEncode(this.nameListTag_, FabricExport.NAME_LIST_TAG_SIZE);
        ...
        for (int i = max_index; i >= 0; i--) {
            if (list_entry_array[i] != null) {
                if (this.nameList_.length() == 0) {
                    this.nameList_ = Encoders.iEncode(this.nameListTag_, FabricExport.NAME_LIST_TAG_SIZE);
                }
                else {
